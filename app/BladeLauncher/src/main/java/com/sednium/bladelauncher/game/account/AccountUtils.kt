...
package com.sednium.bladelauncher.game.account
...
import com.sednium.bladelauncher.context.COPY_LABEL_DEVICE_CODE
...
import com.sednium.bladelauncher.game.account.auth_server.getAuthServeInfo
import com.sednium.bladelauncher.game.account.microsoft.AsyncStatus
import com.sednium.bladelauncher.game.account.microsoft.AuthType
...
import com.sednium.bladelauncher.game.account.microsoft.fetchDeviceCodeResponse
import com.sednium.bladelauncher.game.account.microsoft.getTokenResponse
import com.sednium.bladelauncher.game.account.microsoft.microsoftAuthAsync
...
fun localLogin(userName: String, userUUID: String?) {
    val cleanUUID = if (!userUUID.isNullOrBlank()) {
        runCatching {
            accountUUID(UUID.fromString(if (userUUID.contains("-")) userUUID else regex.replace(userUUID, "$1-$2-$3-$4-$5")))
        }.getOrElse {
            accountUUID(getUUIDFromUserName(userName))
        }
    } else {
        accountUUID(getUUIDFromUserName(userName))
    }
