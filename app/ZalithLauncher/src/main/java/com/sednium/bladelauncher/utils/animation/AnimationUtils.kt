...
package com.sednium.bladelauncher.utils.animation
...
/**
 * 列表项初始缩放大小 (FADE 与 CLOSE 模式下不进行缩放)
 */
fun getItemInitialScale(): Float = if (AllSettings.launcherSwapAnimateType.state == TransitionAnimationType.SLICE_IN) 0.95f else 1f
...
fun <E> getAnimateTweenBounce(
    delayMillis: Int = 0
): FiniteAnimationSpec<E> = getAnimateTween(delayMillis = delayMillis)
...
fun swapAnimateDpAsState(
    ...
    return if (type == TransitionAnimationType.SLICE_IN) {
