...
package com.sednium.bladelauncher.ui.screens
...
import com.sednium.bladelauncher.utils.animation.getAnimateSpeed
...
fun rememberSwapTween(): FiniteAnimationSpec<Float> {
    ...
    return remember(speed) {
        tween(durationMillis = getAnimateSpeed(), easing = androidx.compose.animation.core.FastOutSlowInEasing)
    }
}
...
fun <T : Any> rememberTransitionSpec(): AnimatedContentTransitionScope<Scene<T>>.() -> ContentTransform {
    ...
    return remember(type, speed) {
        val tween: FiniteAnimationSpec<Float> = when (type) {
            ...
            else -> tween(durationMillis = getAnimateSpeed(), easing = androidx.compose.animation.core.FastOutSlowInEasing)
        }
